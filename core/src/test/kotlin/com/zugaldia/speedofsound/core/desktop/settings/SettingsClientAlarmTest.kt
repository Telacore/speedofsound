...
class SettingsClientAlarmTest {
    ...
    @Test
    fun `setMaxAlarms heals malformed alarms even when the limit is unchanged`() {
        val store = MapSettingsStore(
            initialValues = mutableMapOf(
                KEY_MAX_ALARMS to "5",
    ...
    @Test
    fun `alarm scheduler state heals malformed legacy json`() {
        val store = MapSettingsStore(
            initialValues = mutableMapOf(
        ...
        assertEquals(
            DEFAULT_ALARM_LAST_TRIGGERED_DATES,
            store.getString(KEY_ALARM_LAST_TRIGGERED_DATES, DEFAULT_ALARM_LAST_TRIGGERED_DATES)
        )
        assertEquals(
            DEFAULT_ALARM_LAST_CHECK_AT,
            store.getString(KEY_ALARM_LAST_CHECK_AT, DEFAULT_ALARM_LAST_CHECK_AT)
        )
    }

    @Test
    fun `alarm scheduler state heals partially invalid legacy trigger dates`() {
        val store = MapSettingsStore(
            initialValues = mutableMapOf(
                KEY_ALARM_LAST_TRIGGERED_DATES to Json.encodeToString(
                    mapOf(
                        "alarm-1" to "2026-05-29",
                        "alarm-2" to "not-a-date",
                    )
                ),
                KEY_ALARM_LAST_CHECK_AT to "2026-05-29T09:15:30",
            )
        ...
        assertEquals(
            Json.encodeToString(mapOf("alarm-1" to "2026-05-29")),
            store.getString(KEY_ALARM_LAST_TRIGGERED_DATES, DEFAULT_ALARM_LAST_TRIGGERED_DATES)
        )
        assertEquals(
            "2026-05-29T09:15:30",
    ...
    @Test
    fun `alarm trigger dates setters do not heal malformed legacy state`() {
        val store = MapSettingsStore(
            initialValues = mutableMapOf(
                KEY_ALARM_LAST_TRIGGERED_DATES to "{bad-trigger",
                KEY_ALARM_LAST_CHECK_AT to "{bad-check",
        ...
        assertEquals("{bad-trigger", store.getString(KEY_ALARM_LAST_TRIGGERED_DATES, DEFAULT_ALARM_LAST_TRIGGERED_DATES))
        assertEquals("{bad-check", store.getString(KEY_ALARM_LAST_CHECK_AT, DEFAULT_ALARM_LAST_CHECK_AT))
    }
